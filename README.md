...
The `Geo::Geometry::WKT` module contains a single function:

from-wkt
--------

`from-wkt` takes a single `Str` parameter, abd returns a single `Geo::Geometry` object corresponding to the wkt specification, or a `Failure` if the contents of the string cannot be interpreted as a Geometry object.

The conversion is case-insensitive, and optiona whitespace may be included in various places. Thus the following strings will all produce the same object:
...
The `Geo::Geometry::WKT::Grammar` module contains a single grammar `Geo::Geometry::WKT::Grammar::WKT`.

This is the same grammar used by the `Geo::Geometry::WKT` module, but it can be used separately to parse WKT strings.

The general usage would be

        my $results = Geo::Geometry::WKT::Grammar::WKT.parse($string, actions => My::Actions);

The actions used by `Geo::Geometry::WKT::from-wkt` are available in `Geo::Geometry::WKT::WKT-Actions`, but in that case you might as well use `from-wkt` described above.
